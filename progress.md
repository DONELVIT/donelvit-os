...
- [x] Production release: commit `f1b1f51` deployed as `dpl_GnuyCZjvwcf5NhfL3X75HtoVhDXC` and reached READY with error-only build logs reporting completion. Production `/verification` returned HTTP 200.
- [x] Added the editable internal dossier status and verdict (draft/review/completed/archived; compliant/comments/noncompliant/insufficient data). Changing it uses the existing editor-only RLS policy and does not create an official conclusion.
- [x] Fixed verification-role display after production diagnosis: `user_roles` had an authenticated RLS policy but no authenticated SELECT grant. Applied `grant_authenticated_role_read`; authenticated users can now read only rows allowed by the existing RLS policy, anon remains denied. The sole employee record is verified as `admin`.
- [x] Scope corrected by user: verification is limited to fire alarm, notification and extinguishing systems. The catalog now has one systems-only profile and no longer includes general construction, architecture, occupancy or unrelated object-category checks.
- [x] Added no-cost automatic preliminary analysis for uploaded DOCX files. It runs only in the protected app, reads private DOCX text (maximum 10 MB per file), and creates/update five manual-review findings for alarm, notification, extinguishing, addressing/schemes and power. It does not send files to an external AI service and does not analyse drawings, PDF scans or DWG geometry.
- [x] Updated user-authorised file scope: upload accepts only PDF/DOCX up to 100 MB; automatic preliminary analysis supports text-based PDF and DOCX (up to 500 PDF pages per file). Scanned/image-only PDFs still require OCR/manual review.
