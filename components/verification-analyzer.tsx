...
export function VerificationAnalyzer() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (file.size > 4 * 1024 * 1024) return setError("Для безопасной обработки через приложение размер файла ограничен 4 МБ.");
    ...
    const form = new FormData(); form.set("file", file); form.set("profile", profile); setBusy(true);
    try {
      const response = await fetch("/api/verification/analyze", { method: "POST", headers: { Authorization: `Bearer ${data.session.access_token}` }, body: form });
