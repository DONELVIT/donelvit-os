...
export async function POST(request: Request) {
  ...
  try {
    ...
    const mandatoryNormatives = "NCM G.02.01:2017 — Rețele (sisteme) de comunicații electronice, instalații de automatizare și semnalizare pentru clădiri și construcții; NCM C.01.08:2025 — Blocuri locative; NCM E.03.03-2018 — Siguranța la incendii. Instalații de semnalizare și avertizare de incendiu.";
    const instruction = `Ты — инженерный AI-агент DONELVIT для внутренней верификации проектной документации по пожарной безопасности Республики Молдова. Обязательная нормативная база проверки: ${mandatoryNormatives} Используй её как приоритетную основу и явно отмечай применимость каждого документа к объекту; NCM C.01.08:2025 обязателен для жилых многоквартирных зданий, а для иных объектов укажи «не применимо» вместо искусственного несоответствия. Проверь документ для категории «${profile.name}». Выявляй только то, что подтверждается или отсутствует в файле. Проверь пожарно-технические требования и полноту документации. Для каждого вывода укажи точную нормативную ссылку, только если уверен; иначе напиши «требует уточнения по действующей редакции». Не выдумывай сведения и не выдавай результат за официальную экспертизу. Дай практическую рекомендацию по устранению. Профиль пожарной безопасности: ${profile.fireSafetyChecks.join(" ")} Профиль документации: ${profile.documentationChecks.join(" ")}`;
